...
const consultationInclude = [{ model: User, as: 'patient', attributes: ['id', 'username', 'email'] }];
...
exports.createConsultation = async (req, res) => {
  try {
    ...
    const hydrated = await Consultation.findByPk(consultation.id, {
      include: consultationInclude,
    });
    res.status(201).json(hydrated || consultation);
  } catch (err) {
...
exports.updateConsultation = async (req, res) => {
  try {
    ...
    res.json(hydrated || consultation);
  } catch (err) {
